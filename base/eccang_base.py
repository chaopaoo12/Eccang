...
def build_connect(json_file, interface_name, biz_content):
    ...
    params = {
        ...
        "service_id": setting.get('service_id'),
        "sign_type": "MD5",
...
def get_data(interface_name, biz_content, data_format='json'):
    ...
    while page <= target_page:
        ...
        params, Secret_Key = build_connect('setting.json', interface_name, biz_content)
        sign_str = mk_str(params, Secret_Key)
        params1 = md5_sign(params, sign_str)
        data = post_request(params1)
        print('request_body: ', data.request.body)
        print('response_body: ',data.text)
